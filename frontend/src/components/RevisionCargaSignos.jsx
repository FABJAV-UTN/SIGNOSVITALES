...
/**
 * Paso de revisión de la carga masiva de signos:
 * 1) confirma los nombres que se parecen a alguien de la base,
 * 2) lista a los que no están y deja tildar a quién crear,
 * 3) crea las personas tildadas y carga los signos.
 */
export default function RevisionCargaSignos({ data, preview, onCancelar, onTerminado }) {
  ...
  async function confirmar() {
    ...
    try {
      ...
      let ok = 0;
      if (filas.length > 0) {
        const res = await api.post("/signos/bulk", {
          operativo_id: data.operativo_id ?? null,
          lugar_custom: data.lugar_custom ?? null,
          rows: filas,
        });
        ok = res.data.ok;
